                ...
                <div class="col-12 order-1">
                    <div class="row justify-content-center align-items-center">
                        <div class="col-lg-7 text-center pt-lg">
                            ...
                            <p class="lead text-primary">{{ questions.length + ' Từ vựng' }}</p>
                            <div class="btn-wrapper">
                                <button type="" class="btn mb-3 mb-sm-0 btn-icon btn--primary pl-5 pr-5" @click="getStarted()">
                                    <span v-if="questions.length > 0">
                                        <i class="fas fa-paper-plane"></i> Bắt đầu
                                    </span>
                                    <span v-else>
                                        <i class="fas fa-paper-plane"></i> Khởi động
                                    </span>
                                </button>
                            </div>
                        </div>
        ...
        <div v-if="started" :loading="loading">
                ...
                <div class="card text-center">
                    ...
                    <div class="card-body">
                        <div v-for="(item, index) in answers" :key="index" class="w-100">
                            <button :class="'btn btn-answer w-100 max-w btn-' + getAnswerType(item)" :disabled="answered" @click="selectAnswer(item)">
                                <span class="badge badge-pill badge-info d-only mr-2">{{ index + 1 }}</span>
                                {{ getAnswerLabel(item) }}
                            </button>
                        </div>
                    </div>
                    ...
                    <div class="card-body">
                        <div class="text-center">
                            <div class="custom-control custom-checkbox mb-3">
                              <input v-model="form.sound" type="checkbox" class="custom-control-input" id="audioCheckbox">
                              <label class="custom-control-label" for="audioCheckbox"><i class="ni ni-headphones"></i> Âm thanh</label>
                            </div>
                            <span class="mr-3"><span class="badge badge-pill badge-info">Alt</span>: Phát âm</span>
                            <span v-if="step === steps.review" class="mr-3"><span class="badge badge-pill badge-info mr-2">Space</span>: Xoay thẻ</span>
                            <span v-if="step === steps.listen && !answered" class="mr-3"><span class="badge badge-pill badge-info mr-2">Enter</span>: Trả lời</span>
                            <span v-if="answered" class="mr-3"><span class="badge badge-pill badge-info mr-2">Enter</span>: Tiếp tục</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="row footer">
                <div class="card text-center w-100">
                    <div class="card-body">
                        <div class="text-center">
                            <button v-if="!answered" class="btn btn--primary w-80 max-w" :disabled="!answer" @click="checkListenAnswer">
                                <i class="fas fa-check"></i> Kiểm tra
                            </button>
                            <div v-else class="">
                                <button v-if="!completed" class="btn btn--primary w-80 max-w" @click="nextStep">
    ...
    export default {
        ...
        methods: {
            ...
            async fetchQuestions() {
                this.loading = true
                let URL = this.wordsApi
                if (this.wordsCount !== this.remembersCount) {
            ...
            getQuestion() {
                ...
                this.answer = null
                this.answerStatus = DEFAULT_STATUS
                this.isPlayCorrect = false
                this.isListenCorrect = false
                ...
                this.preLoadAudio(this.questions[this.currentQuestion + 1])
            },
            flipCard() {
            ...
            checkListenAnswer() {
                ...
                if (answerStr === wordStr) {
                    ...
                    this.listenCorrect++
                    this.isListenCorrect = true
                    this.postCorrectAnswer()
                } else {
            ...
            postCorrectAnswer() {
                ...
                if (this.isPlayCorrect && this.isListenCorrect) {
                    return axios
                        .post(this.answerApi, {
            ...
            setRatePercent() {
                this.ratePercent = (this.listenCorrect + this.playCorrect) * 100 / (this.questions.length * 2)
            },
            getStarColor(min) {
            ...
            toggle () {
                this.$fullscreen.toggle(this.$el.querySelector('.main-content'), {
                    wrap: false,
